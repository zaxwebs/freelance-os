					...
					<div class="divide-y divide-border/70">
						{#each data.transactions as transaction (transaction.id)}
							<div class="flex items-center gap-3 px-4 py-3 first:pt-0 sm:px-5">
								...
								<div class="shrink-0 text-right"><p class={`text-sm font-medium ${transaction.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-foreground'}`}>{transaction.type === 'income' ? '+' : '−'}{formatAmountWithCode(transaction.amount, transaction.currency_code)}</p><p class="mt-0.5 text-[11px] text-muted-foreground">{formatAmountWithCode(transaction.displayAmount, data.displayCurrency)} reporting</p></div>
							</div>
						{/each}
					</div>

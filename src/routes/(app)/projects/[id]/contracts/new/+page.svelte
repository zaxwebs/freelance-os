<script lang="ts">
	...
	let selectedTemplate = $derived(data.templates.find((template) => template.id === selectedTemplateId));
	let placeholderValues = $derived({ client_name: data.clientName, project_name: data.project.name, freelancer_name: data.freelancerName });
	let placeholderSummary = $derived(selectedTemplate ? analyzeContractPlaceholders(selectedTemplate.content, placeholderValues) : null);
</script>
...
<div class="space-y-5">
	...
	<form method="POST" action="?/createContract" class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
		...
		<div class="space-y-4">
			...
			{#if placeholderSummary}<ContractPlaceholderNotice summary={placeholderSummary} showReady />{/if}
		</div>

	...
	<div class="space-y-4">
		...
		<section class="overflow-hidden rounded-md border border-border bg-card">
			...
			<div class="border-t border-border px-4 py-3">
				{#if data.contract}
					<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div class="min-w-0"><p class="truncate text-sm font-semibold">{data.contract.name}</p><p class="mt-1 text-xs text-muted-foreground">Agreement attached to this project.</p></div><div class="flex flex-wrap gap-2"><Button href={`/projects/${project.id}/contracts/${data.contract.id}`} variant="outline" size="sm">View contract</Button><Button href={`/projects/${project.id}/contracts/${data.contract.id}/edit`} size="sm"><FilePenLine class="size-3.5" /> Edit</Button></div></div>
				{:else}

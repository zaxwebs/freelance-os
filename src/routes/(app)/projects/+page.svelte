...
<div class="space-y-6">
	...
	<section class="grid gap-px overflow-hidden rounded-md border border-border bg-border sm:grid-cols-3"><MetricCard label="All projects" value={data.pagination.total} detail="Workspace total" /><MetricCard label="Active now" value={data.metrics.activeProjects} detail="In motion" tone="primary" /><MetricCard label="Tasks completed" value={data.metrics.completedTasks} detail="Across projects" tone="emerald" /></section>
	...
	{:else}
		...
		<div class="overflow-hidden rounded-md border border-border bg-card">
			...
			{#each data.projects as project (project.id)}
				...
				<a href={`/projects/${project.id}`} class="group grid gap-3 border-b border-border px-4 py-3.5 last:border-0 transition-colors hover:bg-muted/40 sm:grid-cols-[minmax(0,1.4fr)_150px_minmax(180px,0.8fr)_80px] sm:items-center sm:gap-4"><div class="flex min-w-0 items-center gap-3"><div class="flex size-8 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary"><FolderKanban class="size-4" /></div><div class="min-w-0"><p class="truncate text-sm font-medium group-hover:text-primary">{project.name}</p><p class="mt-0.5 truncate text-xs text-muted-foreground">{clientName(project.client_id)}</p></div></div><div><span class="text-[10px] font-semibold tracking-[0.1em] text-muted-foreground uppercase sm:hidden">Status · </span><Badge class={statusClass(project.status)}>{statusLabel(project.status)}</Badge></div><div><div class="flex items-center justify-between text-xs text-muted-foreground"><span>{done} of {total} done</span><span class="font-medium text-foreground">{progress}%</span></div><div class="mt-1.5 h-1 overflow-hidden rounded-full bg-muted"><div class="h-1 rounded-full bg-primary transition-all" style={`width: ${progress}%`}></div></div></div><div class="flex items-center justify-between text-xs text-muted-foreground"><span>{total} {total === 1 ? 'task' : 'tasks'}</span><ArrowUpRight class="size-3.5 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></div></a>
			{/each}
		</div>
		<PaginationControls basePath="/projects" page={data.pagination.page} pageSize={data.pagination.pageSize} total={data.pagination.total} />
		</div>
	{/if}
</div>

<script lang="ts">
	...
	let visibleClients = $derived(data.clients.filter((client) => `${client.name} ${client.company ?? ''}`.toLowerCase().includes(data.query.toLowerCase())));
...
<div class="space-y-6">
	...
	<section class="grid gap-px overflow-hidden rounded-md border border-border bg-border sm:grid-cols-3"><MetricCard label="Clients" value={data.clients.length} detail="Workspace total" icon={Users} tone="primary" /><MetricCard label="Projects" value={data.projects.length} detail="Across clients" icon={FolderKanban} /><MetricCard label="Tracked tasks" value={data.tasks.length} detail="Connected work" icon={ListChecks} tone="emerald" /></section>

...
export const load: PageServerLoad = async ({ locals: { supabase }, url }) => {
	...
	if (validStatuses.includes(requestedStatus)) pageCountQuery.eq('status', requestedStatus);
	...
	if (validStatuses.includes(requestedStatus)) pageQuery.eq('status', requestedStatus);

		...
		<CardContent class="space-y-4 p-4 sm:p-5">
			...
			<form method="POST" action="?/saveInvoiceSettings" use:enhance class="space-y-4">
				...
				<div class="grid gap-4 sm:grid-cols-2">
					...
					<div class="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3"><div class="space-y-1.5"><Label for="tax-id-label">Tax ID label <span class="font-normal text-muted-foreground">optional</span></Label><Input id="tax-id-label" name="tax_id_label" value={data.invoiceSettings.tax_id_label ?? ''} placeholder="VAT number" /></div><div class="space-y-1.5"><Label for="tax-id">Tax ID <span class="font-normal text-muted-foreground">optional</span></Label><Input id="tax-id" name="tax_id" value={data.invoiceSettings.tax_id ?? ''} placeholder="Tax number" /></div></div>
				</div>
				<div class="grid gap-4 sm:grid-cols-2">
